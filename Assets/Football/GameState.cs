        ...
        private void UpdateLeftScoreText(int previousValue, int newValue)
        {
            ...
            if (IsServer)
            {
                if (newValue == 5)
                {
                    ...
                    footballGame.SetWinnerServerRpc(winner.GetComponent<SteamPlayer>().playerName, winner.GetComponent<NetworkObject>().OwnerClientId, loser.GetComponent<NetworkObject>().OwnerClientId);
